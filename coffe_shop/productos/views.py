...
class ListProductsViews(generic.ListView):  # lista generica ListView
    ...
    context_object_name = (
        "products"  # -- este nombre se utiliza en el archivo template html
    )
...
class ProductFormView(generic.FormView):
    ...
    success_url = reverse_lazy(
        "success"
    )  # cuando el producto se cree, haga un redireccionamiento a la url especificada
...
class success(TemplateView):
    ...
    def success(request):
        all_products = (
            Product.objects.all()
        )  # .objects.all() obtiene todos los atributos de Product
